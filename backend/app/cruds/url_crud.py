...
def get_url_by_code(db: Session, code: str):
    return db.query(URL).filter(URL.code == code, URL.delete_at > datetime.now(timezone.utc)).first()

# Retrieve all URL entries from the database for a specific user
def get_urls_by_user(db: Session, user_id: int):
    return db.query(URL).filter(URL.created_by == user_id, URL.delete_at > datetime.now(timezone.utc)).order_by(URL.created_at.desc()).all()
